    ...
    <div @if (FilamentView::hasSpaMode()) ax-load="visible"
        ...
        @if ($hasImageEditor && !$isDisabled)
            <div x-show="isEditorOpen" x-cloak x-on:click.stop="" x-trap.noscroll="isEditorOpen"
                x-on:keydown.escape.window="closeEditor" @class([
                ...
                <div class="isolate z-10 flex h-full w-full items-center justify-center">
                    <div
                        class="mx-auto flex h-full w-full flex-col overflow-hidden rounded-xl bg-white ring-1 ring-gray-900/10 dark:bg-gray-900 dark:ring-gray-50/10 lg:flex-row">
        ...
        [
            ...
            'alpineSaveHandler' => 'editor.rotateTo(+$el.value)',
                                                    ...
                                                    <label
                                                        class="flex w-full items-center rounded-lg border border-gray-300 bg-gray-100 text-sm shadow-sm dark:border-gray-700 dark:bg-gray-900">
                                                        <span
                                                ...
                                                @foreach ($getImageEditorActions(iconSizeClasses: 'h-5 w-5 mx-auto') as $groupedActions)
                                                    <x-filament::button.group class="w-full">
                                                        @foreach ($groupedActions as $action)
                                                            <x-filament::button color="gray" grouped :icon="new \Illuminate\Support\HtmlString(
                                                                $action['iconHtml'],
                                                            )"
                                                                label-sr-only
